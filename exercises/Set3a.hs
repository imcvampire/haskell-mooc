...
interpreter' (_, _) [] out = reverse out
interpreter' (x, y) (c:cs) out = case c of
  "up" -> interpreter' (x, y + 1) cs out
  "down" -> interpreter' (x, y -1) cs out
  "right" -> interpreter' (x + 1, y) cs out
  "left" -> interpreter' (x -1, y) cs out
